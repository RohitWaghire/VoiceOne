...
export function createDubEngine(adapter, { findCueIndex, bcp47For, labelFor, mergeLanguages }) {
  const state = {
    ...
    utterDeadline: 0, // watchdog: when the in-flight utterance is declared dead
  ...
  // Some players (Bilibili quality switches, DASH re-inits) replace their
  // <video> element mid-play. The old element detaches and its clock freezes,
  // which used to stall the dub silently — carry the dub over to the live
  // element instead. savedVolume is kept: it is the user's pre-dub level, and
  // sites set a fresh element back to the user's own volume, not our duck.
  function rebindVideo(video) {
    detachVideo(state.video);
    state.video = video;
    applyDuck();
    attachVideo(video);
    state.idx = findCueIndex(state.cues, video.currentTime * 1000);
  }
  ...
  function tick() {
    ...
    // Same video, replaced element (quality switch) → follow it. The explicit
    // `=== false` matters: harness fakes aren't DOM nodes and lack isConnected.
    if (state.video.isConnected === false) {
      const live = adapter.getVideo();
      if (!live || live === state.video) return; // player rebuilding; retry next tick
      rebindVideo(live);
    }
    // Chrome's speechSynthesis can wedge: audio stops but neither onend nor
    // onerror ever fires, which would leave `uttering` blocking the dub
    // forever. resume() un-sticks the auto-paused flavor of the bug; past the
    // deadline (armed in speak() from the expected duration) declare it dead.
    if (state.uttering) {
      if (window.speechSynthesis.paused) window.speechSynthesis.resume();
      if (performance.now() > state.utterDeadline) {
        window.speechSynthesis.cancel();
        state.uttering = false;
      }
    }
  ...
  function speak(cue, nowMs) {
    ...
    u.rate = rate;
    u.volume = 1;
    // Arm the tick watchdog: expected speech time plus generous slack for
    // queueing/voice startup; re-armed tighter once the utterance really
    // starts. If neither onend nor onerror ever fires (Chrome synth wedge),
    // the deadline is what recovers the dub.
    const expectMs = estMs / rate;
    state.utterDeadline = performance.now() + expectMs * 1.5 + 5000;
    let startedAt = 0;
    u.onstart = () => {
      startedAt = performance.now();
      state.utterDeadline = startedAt + expectMs * 1.5 + 3000;
    };
    u.onend = u.onerror = (e) => {
      state.uttering = false;
